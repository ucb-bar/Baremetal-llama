...
// type alias ConfigInt is int16_t
typedef int32_t ConfigInt;

typedef struct {
    ConfigInt dim; // transformer dimension
    ConfigInt hidden_dim; // for ffn layers
    ConfigInt n_layers; // number of layers
    ConfigInt n_heads; // number of query heads
    ConfigInt n_kv_heads; // number of key/value heads (can be < query heads because of multiquery)
    ConfigInt vocab_size; // vocabulary size, usually 256 (byte-level)
    ConfigInt seq_len; // max sequence length
} Config;
...
void build_tokenizer(Tokenizer* t, int vocab_size) {
    ...
    if (vocab_size != t->vocab_size) { 
        printf("vocab size %d does not match %d\n", vocab_size, t->vocab_size);
        exit(EXIT_FAILURE); 
    }
